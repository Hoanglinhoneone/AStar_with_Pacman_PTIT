...
def chosenAl(idx,_map, start_row, start_col, end_row, end_col, N, M):
    if idx%2 == 0:
        return Astar(_map, start_row, start_col, end_row, end_col, N, M)
    else:
        return Astar(_map, start_row, start_col, end_row, end_col, N, M)
...
def generate_Ghost_new_position(_ghost, _type: int = 0) -> list[list[int]]:
    ...
    # update latest
        ...
        for idx in range(len(_ghost)):
            ...
            _ghost_new_position.append(chosenAl(idx,_map, start_row, start_col, end_row, end_col, N, M))
    ...
    while not done:
        ...
        if delay <= 0:
            if is_moving:
                ...
                #new_PacMan_Pos = minimaxAgent(_map, row, col, N, M, depth=4, Score=Score)
